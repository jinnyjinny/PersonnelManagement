...
public: // 기반클래스인 Time_Oper에서 쓰이는 public멤버변수를 초기화한다. 
	void SET_Day(int day) { // SET_Day의 기능을 하는 메소드를 정의한다. this->day의 의미: day변수를 다른 클래스에서도 쓰기위해?  
		this->day = day;
	}
	int GET_Day() { return day; } //return값의 의미는?-> 값을 반환한다. 계산된 day라는 값을 갖고 호출된 부분으로 다시 돌아감
